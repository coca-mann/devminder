...
export default function ProjetoViewPage() {
  ...
  const handleToggleTaskTag = (taskId: string, tag: string) => {
    setProjectTasks(currentTasks =>
      currentTasks.map(task => {
        if (task.id === taskId) {
          const newTags = task.tags.includes(tag)
            ...
            : [...task.tags, tag];
          // Also update the selectedTask state if it's the one being changed
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="tasks" className="mt-6 space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <Sheet>
                    <SheetTrigger asChild>
                      <Button variant="outline" className="md:hidden" disabled={!selectedTask}>
                    ...
                    <SheetContent side="bottom">
                      <SheetHeader>
                        <SheetTitle>Adicionar tags a "{selectedTask?.title}"</SheetTitle>
                        <SheetDescription>Toque em uma tag para adicioná-la ou removê-la.</SheetDescription>
                      </SheetHeader>
                      <div className="py-4 flex flex-wrap justify-center gap-2">
                        {allTags.map(tag => (
                          <Badge
                            ...
                            className={cn('cursor-pointer text-sm py-1 px-3', tagColors[tag])}
                            onClick={() => {
                              if (selectedTask) {
                                handleToggleTaskTag(selectedTask.id, tag);
                              }
                            }}
                          >
                            {tag}
            ...
            <TaskList 
              tasks={projectTasks} 
              onUpdateTaskTags={handleAddTaskTag} 
